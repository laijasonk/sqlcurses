...
def open_front_main(scr_dim):
    ...
    if scr_dim[1] > 64:
        ...
        scr_front_main.addstr(
            3, 2, "Make sure to open the database from the command line"
        )
        scr_front_main.addstr(4, 2, "e.g. sqlcurses -t sqlite -d demo.sqlite3")
        scr_front_main.addstr(
            5, 2, "e.g. sqlcurses -t postgres -d demo -u johnsmith -h localhost"
        )
        scr_front_main.addstr(
            5,
            2,
            "e.g. sqlcurses -t mysql -d dev -u john -p pass -h devc -s /tmp/mysql.sock",
        )
        ...
        if scr_dim[0] > 25:
            scr_front_main.addstr(
                19, 2, "Thank you for using SQLcurses! This was made to allow SQL"
            )
            scr_front_main.addstr(
                20, 2, "database manipulation to be done right in the console."
            )
...
def create_environment():
    ...
    if not os.path.exists(root_path + "/.sqlcurses"):
        ...
        try:
            os.system(
                "cp "
                + os.path.dirname(sqlcurses.__file__)
                + "/saved_databases "
                + root_path
                + "/.sqlcurses/saved_databases"
            )
        except:
            pass
